...
object CommonUtils {
    ...
    object MovieCoverters {
        ...
        fun toMovieEntityWithGroup(movie: MovieModel, group: String) {
            toMovieEntity(
                MovieEntity(
                    ...
                    cast = movie.credits.cast.map { CastConverters.toCastEntity(it) },
                    groupmovie = group
                )
            )
        }
        ...
        @TypeConverter
        fun toMovieEntity(movie: MovieEntity) = movie
    }

    object CastConverters {
        fun toCastEntityList(list: List<Cast>): List<CastEntity> {
            return list.map { toCastEntity(it) }
        }
        ...
        fun toCastEntity(cast: Cast) = CastEntity(
            adult = cast.adult,
            cast_id = cast.id,
